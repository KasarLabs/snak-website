...
export default function HomePage() {
	...
	const [mousePosition, setMousePosition] = useState({ x: windowSize.width / 2, y: windowSize.height / 2 });
	const [dragPosition, setDragPosition] = useState({ x: 0, y: 0 });
	const [isDragging, setIsDragging] = useState(false);
	const [frozenMousePosition, setFrozenMousePosition] = useState({ x: windowSize.width / 2, y: windowSize.height / 2 });
	...
	const generateHoneycombPositions = (totalItems: number, width: number, height: number) => {
		const positions = [];
		...
		return positions.sort((a, b) => {
			const distA = Math.sqrt(Math.pow(a.x - width/2, 2) + Math.pow(a.y - height/2, 2));
			const distB = Math.sqrt(Math.pow(b.x - width/2, 2) + Math.pow(b.y - height/2, 2));
			return distA - distB;
		}).slice(0, totalItems);
	};
	...
	const circles = useMemo(() => {
		...
		const positions = generateHoneycombPositions(totalItems, windowSize.width, windowSize.height);
	...
	const renderItem = (circle: GridItem) => {
		...
		return (
		<div className="w-full h-full rounded-full" style={{backgroundColor: circle.color,}}/>
		);
	};
	...
	return (
		<div ref={containerRef} onMouseMove={handleMouseMove }className="relative w-full h-screen bg-black overflow-hidden">
			<div className="relative w-full h-full px-5">
				...
				>
					{visibleCircles.map((circle) => {
						const activePosition = (isDragging || isModalVisible) ? frozenMousePosition : mousePosition;
						const circleCenter = {
							x:circle.x  + RADIUS / 2,
							y:circle.y  + RADIUS / 2
						};
						...
						const scale = distanceFromMouse < ZOOM_ZONE_SIZE / 2
							? 1 + (CENTER_ZOOM - 1) * Math.max(1 - distanceFromMouse / (ZOOM_ZONE_SIZE / 2))
							: 1;
						return (
							<motion.div
								key={circle.id}
